...
```{r}
p_interference_treatment <- mental_health %>%
  filter(!is.na(work_interfere), !is.na(treatment)) %>%
  ggplot(aes(x = work_interfere, fill = treatment)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(
...
Company policies have a small effect on whether an employee gets treated and treated employees report lower levels of intererence with work due to mental health.

```{r}
p1 <- mental_health %>%
  filter(!is.na(treatment), !is.na(work_interfere)) %>%
  ggplot(aes(x = treatment, fill = work_interfere)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  scale_fill_brewer(palette = "RdYlBu", direction = -1) +
  labs(
    title = "Work Interference by Mental Health Treatment Status",
    subtitle = "Treated individuals report less frequent work interference",
    x = "Treatment Status",
    y = "Proportion",
...
p2 <- mental_health %>%
  ...
  labs(
    ...
    subtitle = "Gender differences in how benefits affect mental health at work",
    x = "Company Provides Mental Health Benefits",
    y = "Proportion",
    fill = "Work Interference"
  ) +
...
Company Mental Healty policies according to analysis of this 2014 survey have little to no effect on productivity.